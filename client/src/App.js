...
const PROD_URL = "https://connect4-backend-mgrh.onrender.com"; // Your Render URL

const API_BASE = IS_PRODUCTION ? PROD_URL : "http://localhost:8080";
const WS_BASE = IS_PRODUCTION
  ? PROD_URL.replace(/^http/, "ws")
  : "ws://localhost:8080";
// If you haven't deployed a separate analytics service, this will point to the main backend in prod
// (It might return 404 for stats, but won't crash the app)
const ANALYTICS_BASE = IS_PRODUCTION ? PROD_URL : "http://localhost:8081";
...
const AnalyticsModal = ({ onClose }) => {
  ...
  useEffect(() => {
    // UPDATED: Uses ANALYTICS_BASE instead of localhost
    fetch(`${ANALYTICS_BASE}/stats`)
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setLoading(false);
...
function App() {
  ...
  const fetchLeaderboard = async () => {
    try {
      // UPDATED: Uses API_BASE instead of localhost
  ...
  const connect = () => {
    ...
    // UPDATED: Uses WS_BASE instead of localhost
    socketRef.current = new WebSocket(`${WS_BASE}/ws?username=${username}`);
    socketRef.current.onopen = () => setConnected(true);
    socketRef.current.onmessage = (event) => {
      const msg = JSON.parse(event.data);
      if (msg.type === "START") {
